...
> **Règle impérative de migration.** La branche de travail part toujours de
> `upstream/main` (ou du dernier tag officiel), puis reçoit les commits/features
> PK par-dessus : `version officielle à jour → réapplication des features PK`.
> Ne jamais cherry-picker l’amont sur une branche PK ancienne. Avant toute
> reprise : sauvegarde, `git fetch upstream`, repositionnement sur
> `upstream/main`, puis replay des commits PK et validation TypeScript.

La base amont est configurable avec `PK_FEATURE_BASE`. Elle peut être un tag, une branche ou un SHA. La base officielle de cette reprise est `upstream/main` (`04711a1`), issue de `v0.1.53` (`5adc76a`).
